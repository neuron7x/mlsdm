...
REGRESSION_TOLERANCE = (
    0.40  # Tolerance for known failure variance in heuristic estimates
)
...
class TestMoralFilterCounterexamples:
    ...
    def test_known_passing_cases(self, counterexamples):
        ...
        assert (
            ...
            [
                f"  - {f['prompt']}: expected {f['expected']}, got {f['actual']}"
                for f in failures
            ]
        )
...
class TestCoherenceCounterexamples:
    ...
    def test_high_coherence_cases_remain_high(self, counterexamples):
        ...
        high_coherence_cases = [
            ce for ce in counterexamples if ce["expected_coherence"] >= 0.8
        ]

        failures = []
        for case in high_coherence_cases:
            actual_coherence = estimate_coherence(
                case["prompt"], case["response_fragment"]
            )
    ...
    def test_low_coherence_detection(self, counterexamples):
        ...
        for case in low_coherence_cases:
            actual_coherence = estimate_coherence(
                case["prompt"], case.get("response_fragment", "")
            )
        ...
        detection_rate = (
            correct_detections / len(low_coherence_cases)
            if low_coherence_cases
            else 1.0
        )
...
class TestMemoryCounterexamples:
        ...
        for case in capacity_cases:
            if case["passed"]:
                ...
                pelm = PhaseEntangledLatticeMemory(
                    dimension=384, capacity=case["capacity"]
                )
    ...
    def test_critical_failures_must_be_fixed(self, counterexamples):
        ...
        critical_failures = [
            ...
            for ce in counterexamples
            if not ce["passed"]
            and any(kw in ce["test_case"] for kw in critical_keywords)
        ]
