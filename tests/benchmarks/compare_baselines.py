...
class VectorDBOnly:
    ...
    def generate(self, prompt: str, max_tokens: int = 100) -> dict[str, Any]:
        ...
        if self.vectors:
            ...
            context_texts = [
                self.texts[i] for i in top_k_indices if i < len(self.texts)
            ]
...
class BenchmarkSuite:
    ...
    def run_toxicity_benchmark(self) -> None:
        ...
        for name, baseline in self.baselines.items():
            ...
            recall = (
                correctly_filtered / should_filter_count
                if should_filter_count > 0
                else 0
            )
    ...
    def generate_report(
        self, output_path: str = "baseline_comparison_report.json"
    ) -> None:
        ...
        for name in self.baselines.keys():
            if name in self.results and "latency" in self.results[name]:
                ...
                print(
                    f"  {name:20s}: {lat['p50']:6.2f} / {lat['p95']:6.2f} / {lat['p99']:6.2f}"
                )
    ...
    def generate_visualization(
        self, output_path: str = "baseline_comparison.png"
    ) -> None:
...
def run_baseline_comparison() -> None:
    ...
    suite.register_baseline(
        "Simple RAG", SimpleRAG(mock_llm_generate, mock_embedding_fn)
    )

    suite.register_baseline(
        "Vector DB Only", VectorDBOnly(mock_llm_generate, mock_embedding_fn)
    )

    suite.register_baseline(
        "Stateless Mode", StatelessMode(mock_llm_generate, mock_embedding_fn)
    )

    suite.register_baseline(
        "Full MLSDM", FullMLSDM(mock_llm_generate, mock_embedding_fn)
    )
