...
def _build_provenance(
    content: str,
    *,
    source: MemorySource = MemorySource.SYSTEM_PROMPT,
    confidence: float = 0.95,
    trust_tier: int = 3,
    source_id: str = "test-source",
    ingestion_path: str = "tests.unit.sqlite_store",
...
class TestSQLiteMemoryStorePut:
    ...
    def test_put_basic_item(self, tmp_path):
        ...
        item = MemoryItem(
            ...
            provenance=_build_provenance("Hello world"),
        )
    ...
    def test_put_rejects_missing_provenance(self, tmp_path):
        """Test that store rejects items without provenance."""
        db_path = str(tmp_path / "test.db")
        store = SQLiteMemoryStore(db_path=db_path)
        ...
        item = MemoryItem(
            ...
            content_hash=compute_content_hash("Missing provenance"),
        )
    ...
    def test_put_with_pii_scrubbing(self, tmp_path):
        ...
        item = MemoryItem(
            ...
            provenance=_build_provenance("Contact me at test@example.com for details"),
        )
    ...
    def test_put_with_ttl(self, tmp_path):
        ...
        item = MemoryItem(
            ...
            provenance=_build_provenance("Temporary content"),
        )
    ...
    def test_put_with_pii_flags(self, tmp_path):
        ...
        item = MemoryItem(
            ...
            pii_flags=pii_flags,
            provenance=_build_provenance("Content with PII flags"),
        )
    ...
    def test_put_upsert_existing(self, tmp_path):
        ...
        item1 = MemoryItem(
            ...
            provenance=_build_provenance("Original content"),
        )
        store.put(item1)
        ...
        item2 = MemoryItem(
            ...
            provenance=_build_provenance("Updated content"),
        )
        store.put(item2)
...
class TestSQLiteMemoryStoreGet:
    ...
    def test_get_existing_item(self, tmp_path):
        ...
        item = MemoryItem(
            ...
            provenance=_build_provenance("Content to retrieve"),
        )
        store.put(item)
    ...
    def test_query_basic_text_search(self, tmp_path):
        ...
        for i, content in enumerate(["Hello world", "Goodbye world", "Hello universe"]):
            item = MemoryItem(
                ...
                content=content,
                content_hash=compute_content_hash(content),
                provenance=_build_provenance(content),
            )
            store.put(item)
    ...
    def test_query_with_limit(self, tmp_path):
        ...
        for i in range(10):
            item = MemoryItem(
                ...
                provenance=_build_provenance(f"Test content {i}"),
            )
            store.put(item)
    ...
    def test_query_with_since_ts(self, tmp_path):
        ...
        for i in range(5):
            item = MemoryItem(
                ...
                provenance=_build_provenance(f"Content at time {i}"),
            )
            store.put(item)
    ...
    def test_query_returns_ordered_by_recency(self, tmp_path):
        ...
        for i in range(3):
            item = MemoryItem(
                ...
                provenance=_build_provenance("Search term"),
            )
            store.put(item)
...
class TestSQLiteMemoryStoreEviction:
    ...
    def test_evict_expired_items(self, tmp_path):
        ...
        expired_item = MemoryItem(
            ...
            provenance=_build_provenance("Expired content"),
        )
        ...
        valid_item = MemoryItem(
            ...
            ttl_s=3600.0,  # 1 hour TTL
            provenance=_build_provenance("Valid content"),
        )
        ...
        permanent_item = MemoryItem(
            ...
            provenance=_build_provenance("Permanent content"),
        )
    ...
    def test_evict_expired_returns_zero_when_none_expired(self, tmp_path):
        ...
        item = MemoryItem(
            ...
            ttl_s=3600.0,
            provenance=_build_provenance("Fresh content"),
        )
        store.put(item)
    ...
    def test_compact_runs_without_error(self, tmp_path):
        ...
        for i in range(10):
            item = MemoryItem(
                ...
                provenance=_build_provenance(f"Temporary content {i}"),
            )
            store.put(item)
...
class TestSQLiteMemoryStoreSchemaInit:
    ...
    def test_schema_created_on_first_use(self, tmp_path):
        ...
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='provenance_log'"
        )
        assert cursor.fetchone() is not None
