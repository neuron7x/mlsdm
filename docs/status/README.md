...
- **When to update:** Update `docs/status/READINESS.md` whenever changes touch `src/`, `tests/`, configuration files (`config/`), any `Dockerfile*`, or `.github/workflows/`. Include dated evidence (tests, CI jobs, commands) for any status shift.
