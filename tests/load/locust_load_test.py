...
class MetricsCollector:
    ...
    def record_request(self, response_time: float, success: bool) -> None:
        ...
        if current_time - self.last_rps_time >= 1.0:
            rps = (self.request_count - self.last_rps_count) / (
                current_time - self.last_rps_time
            )
    ...
    def determine_saturation_point(self) -> dict[str, Any]:
        ...
        for i in range(0, int(max_time), window_size):
            ...
            window_rps = [
                rps for ts, rps in self.rps_samples if window_start <= ts < window_end
            ]
            if window_rps:
                ...
                window_latencies = [
                    lat
                    for lat, ts in zip(
                        self.latencies[:min_len], self.timestamps[:min_len], strict=True
                    )
