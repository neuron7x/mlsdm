...
| CI / quality gates (coverage, property tests) | NOT VERIFIED | `.github/workflows/readiness.yml` (jobs: unit, coverage_gate), `.github/workflows/property-tests.yml`, `coverage_gate.sh` | Evidence jobs defined in readiness.yml; awaiting current run artifacts for this PR. |
...
- Unit tests: NOT VERIFIED — Evidence: `.github/workflows/readiness.yml` (job: unit, command: `python -m pytest tests/unit -q --disable-warnings --maxfail=1 --junitxml=reports/junit-unit.xml`, artifact: readiness-unit); awaiting current PR run.
...
- Coverage gate: NOT VERIFIED — Evidence: `.github/workflows/readiness.yml` (job: coverage_gate, command: `bash ./coverage_gate.sh`, env `PYTEST_ARGS="--ignore=tests/gpu --ignore=tests/neurolang --ignore=tests/embeddings"`, artifacts: readiness-coverage); awaiting run.
- Security-lite: NOT VERIFIED — Evidence: `.github/workflows/readiness-evidence.yml` (job: security-lite, command: `ruff check . --exit-zero`); awaiting run.
...
## Known Blocking Gaps
1. Evidence jobs pending execution: `.github/workflows/readiness.yml` (unit, coverage_gate) need a passing run with artifacts for this commit.
2. Coverage gate unverified: `bash ./coverage_gate.sh` (readiness.yml job: coverage_gate) not yet executed successfully in CI; need coverage.xml/log artifacts.
3. Integration and property suites skipped in PR runs: need workflow_dispatch run of `.github/workflows/readiness-evidence.yml` executing `python -m pytest tests/integration -q --disable-warnings --maxfail=1` and `python -m pytest tests/property -q --maxfail=3`, with artifacts.
