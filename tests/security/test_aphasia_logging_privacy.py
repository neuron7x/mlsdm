...
def test_aphasia_logs_do_not_contain_prompt_text(caplog):
    ...
    assert (
        SECRET_PROMPT_TOKEN not in caplog.text
...
def test_aphasia_logs_do_not_contain_response_text(caplog):
    ...
    assert (
        SECRET_RESPONSE_TOKEN not in caplog.text
...
def test_aphasia_logs_with_repair_do_not_leak_content(caplog):
    ...
    _ = wrapper.generate(
        prompt=f"Explain {SECRET_PROMPT_TOKEN}", moral_value=0.7, max_tokens=50
    )
...
def test_multiple_generations_do_not_leak_any_secrets(caplog):
    ...
    for secret in secrets:
        _ = wrapper.generate(
            prompt=f"Query about {secret}", moral_value=0.7, max_tokens=50
        )
