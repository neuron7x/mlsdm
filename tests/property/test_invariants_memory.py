...
# Test tolerances
RESONANCE_ORDERING_TOLERANCE = (
    1e-6  # Tolerance for floating point comparison in ordering
)
...
def test_gating_value_bounds(gating12, gating23):
    ...
    memory = MultiLevelSynapticMemory(
        dimension=10, gating12=gating12, gating23=gating23
    )
...
def test_distance_non_increase_after_insertion(dim):
    ...
    L1_after, L2_after, L3_after = memory.get_state()
    final_norm = (
        np.linalg.norm(L1_after) + np.linalg.norm(L2_after) + np.linalg.norm(L3_after)
    )
...
def test_pelm_nearest_neighbor_availability(dim, num_vectors, k):
    ...
    query = np.random.randn(dim).astype(np.float32)
    neighbors = pelm.retrieve(
        query.tolist(), current_phase=phase, phase_tolerance=0.5, top_k=k
    )
...
@settings(max_examples=30, deadline=None)
@given(
    dim=st.integers(min_value=5, max_value=20), k=st.integers(min_value=2, max_value=5)
)
...
def test_empty_memory_query():
    ...
    neighbors = pelm.retrieve(
        query.tolist(), current_phase=0.5, phase_tolerance=0.5, top_k=5
    )
...
def test_single_vector_retrieval():
    ...
    query = np.random.randn(10).astype(np.float32)
    neighbors = pelm.retrieve(
        query.tolist(), current_phase=phase, phase_tolerance=0.5, top_k=5
    )
