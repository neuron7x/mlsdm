...
def test_neurolang_disabled_skips_training_and_grammar():
    ...
    with patch(
        "mlsdm.extensions.neuro_lang_extension.CriticalPeriodTrainer"
...
def test_neurolang_eager_train_calls_trainer_once():
    ...
    with patch(
        "mlsdm.extensions.neuro_lang_extension.CriticalPeriodTrainer.train",
        counting_train,
    ):
        ...
        for _ in range(3):
            result = wrapper.generate(
                prompt="Test prompt", moral_value=0.8, max_tokens=50
            )
            assert result["accepted"] is True
...
def test_neurolang_lazy_train_trains_on_first_generate_only():
    ...
    with patch(
        ...
        result1 = wrapper.generate(
            prompt="First prompt", moral_value=0.8, max_tokens=50
        )
...
def test_neurolang_checkpoint_invalid_format_raises_error():
    ...
    try:
        ...
        with pytest.raises(
            ValueError,
            match="Invalid checkpoint structure.*missing 'actor' or 'critic'",
        ):
            NeuroLangWrapper(
                llm_generate_fn=dummy_llm,
                embedding_fn=dummy_embedder,
