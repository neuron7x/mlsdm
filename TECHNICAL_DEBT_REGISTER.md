...
╠═════════════════════╬═══════════╬═════════════════════════════════════════════╣
║ ВСЬОГО              ║  ~~35~~ ~~33~~ 32║     0    /  ~~5~~ ~~4~~ 3 / ~~13~~ 12 /   17      ║
╠═════════════════════╩═══════════╩═════════════════════════════════════════════╣
║ ✅ ВИРІШЕНО: TD-001 (pip>=25.3 CVE-2025-8869), TD-004 (chaos testing 3→5), TD-018 (Python 3.12) ║
╚══════════════════════════════════════════════════════════════════════════════╝

Статус готовності до продакшену: 94% (↑ 2%)
...
### Високий пріоритет (4 елементи, 2 вирішено)

#### ~~TD-001: Вразливість у pip (CVE-2025-8869)~~ ✅ ВИРІШЕНО
...
| **Локація** | pip toolchain (`pip>=25.3`) |
| **Опис** | ~~Вразливість tarfile path traversal дозволяє перезапис файлів при встановленні шкідливих sdist пакетів~~ → pip 25.3+ включає виправлення |
| **Рішення** | Закріплено `pip>=25.3` у requirements.txt та bootstrap-команді `python -m pip install --upgrade "pip>=25.3"` |
| **Критерій закриття** | `pip --version` показує ≥25.3; pip-audit без вразливостей |
| **Докази** | `pip --version` → 25.3.x; `pip-audit -r requirements.txt` → No known vulnerabilities (системні пакети із базового образу винесені окремо); DL-004 у debt_ledger.md |
...
### Середній пріоритет (12 елементів, 1 вирішено)
